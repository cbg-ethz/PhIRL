...
class MainConfig:
    ...
    data: str = "~/Desktop/Lab_rotation1/PhIRL_data/n_5_N_2000/tree_df.csv"
    n_action: int = 6  # number of mutations + 1
    learning_rate: float = 0.05
    featurizer: Featurizer = Featurizer.IDENTITY
    # featurizer: Featurizer = Featurizer.ONEHOT
    eps: float = 1e-5
    max_iter: int = 1000
...
def main(config: MainConfig) -> None:
    ...
    mdp = me.DeterministicTreeMDP(config.n_action)
    ...
    action_of_trajectories = me.get_action_of_trajectories(trees, config.n_action, max_length=20)
    initial_state = me.initial_state(config.n_action)

    trajectories = me.unroll_trajectories(
        action_trajectories=action_of_trajectories,
